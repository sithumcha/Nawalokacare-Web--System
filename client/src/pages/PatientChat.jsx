...
const PatientChat = () => {
  ...
  useEffect(() => {
    ...
    setUser(userData);
    if (appointmentId) {
      loadAppointmentAndMessages(appointmentId, userData._id);
    } else {
  ...
  const loadAppointmentAndMessages = async (aptId, userId) => {
    try {
      setIsLoading(true);
        ...
        if (appointmentResponse.data.doctorId) {
          try {
            const doctorResponse = await axios.get(
              `http://localhost:5000/api/doctors/${appointmentResponse.data.doctorId}`
            );
            if (doctorResponse.data) {
              setDoctor(doctorResponse.data);
              setDoctorImageError(false); // Reset image error
            }
          } catch (err) {
            console.error("Error fetching doctor details:", err);
            // Set mock doctor if API fails
            setDoctor(generateMockDoctor(appointmentResponse.data));
          }
        }
      }
      ...
      if (messagesResponse.data.success) {
        ...
        await markMessagesAsRead(aptId, userId);
      }
      ...
      setAppointment(generateMockAppointment(userData));
      setDoctor(generateMockDoctor(null, userData));
  ...
  // Start polling for new messages
  const startPolling = useCallback((appointmentId, userId) => {
    ...
    pollingIntervalRef.current = setInterval(async () => {
      try {
        const params = { userId, userType: 'patient' };
        if (lastMessageId) {
        ...
        if (response.data.success && response.data.hasNew) {
          ...
          if (newMessages.length > 0) {
            ...
            // Mark as read
            await markMessagesAsRead(appointmentId, userId);
            
            // Update unread count
            setUnreadCount(prev => Math.max(0, prev - newMessages.length));
          }
        }
  ...
  const sendMessage = async () => {
    if (!newMessage.trim() || !appointment || !user) return;
  ...
  const generateMockAppointment = (userData) => ({
    _id: appointmentId,
  ...
  const generateMockDoctor = (appointmentData, userData) => ({
    _id: "doctor_123",
    firstName: "Apsara",
    lastName: "Chanuka",
    fullName: "Dr. Apsara Chanuka",
    specialization: "Neurology",
    email: "apsara.chanuka@hospital.com",
  ...
  const generateSampleMessages = () => [
    {
      ...
      senderId: "doctor_123",
      senderType: "doctor",
      receiverId: user?._id || "patient_123",
    ...
    },
    {
      ...
      receiverId: user?._id || "patient_123",
      receiverType: "patient",
      content: "That's good to hear. Let's discuss this in our appointment tomorrow.",
  ...
  const handleCallDoctor = () => {
    if (doctor?.phoneNumber) {
      ...
      alert("Doctor's contact number is not available");
    }
  };
          ...
          <div className="flex justify-between items-center py-4">
            ...
            <div className="flex items-center space-x-4">
              <div className="text-right">
                ...
                <p className="font-semibold text-gray-900">{user?.fullName || "Patient"}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-1/3">
                  ...
                  <div className="flex-1">
                    <h3 className="text-xl font-bold text-gray-900">
                      {doctor?.firstName && doctor?.lastName 
                        ...
                        : doctor?.fullName || appointment?.doctorName || "Dr. Unknown"}
                    </h3>
                    <p className="text-blue-600 font-medium">
                      {doctor?.specialization || appointment?.doctorSpecialization || "Specialization"}
                    </p>
                    <div className="flex items-center mt-1">
                      ...
                      <span className="ml-2 text-sm text-gray-600">
                        {doctor?.rating || "4.8"} ({doctor?.experience || "15 years"} experience)
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div className="mt-6 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h4 className="text-lg font-semibold text-gray-900 mb-3">Chat Guidelines</h4>
              <ul className="space-y-2 text-sm text-gray-600">
                ...
                <li className="flex items-start">
                  <span className="text-green-500 mr-2">✓</span>
                  Share any previous medical reports if needed
                </li>
                <li className="flex items-start">
                  <span className="text-green-500 mr-2">✓</span>
                ...
                <li className="flex items-start">
                  <span className="text-green-500 mr-2">✓</span>
                  Ask specific questions about your condition
                </li>
                <li className="flex items-start">
                  <span className="text-green-500 mr-2">✓</span>
                  Respond promptly for better communication
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="lg:w-2/3">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col h-[calc(100vh-200px)]">
              ...
              <div className="p-4 border-b border-gray-200 flex items-center justify-between">
                <div className="flex items-center">
                  {/* Doctor Profile Picture in Chat Header - UPDATED */}
                  <div className="relative w-10 h-10 rounded-full overflow-hidden bg-blue-100 flex-shrink-0">
                    {doctor?.profilePicture && !doctorImageError ? (
                      <img
                        src={`http://localhost:5000/${doctor.profilePicture}`}
                        alt={`${doctor.firstName} ${doctor.lastName}`}
                  ...
                  <div className="ml-3">
                    <h3 className="font-semibold text-gray-900">
                      {doctor?.firstName && doctor?.lastName 
                        ? `Dr. ${doctor.firstName} ${doctor.lastName}`
                        : doctor?.fullName || appointment?.doctorName || "Doctor"}
                    </h3>
                    <div className="flex items-center">
                      <div className={`w-2 h-2 rounded-full mr-2 ${isPolling ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                      <span className="text-xs text-gray-500">
                        {isPolling ? 'Online' : 'Offline'} • {doctor?.specialization || "Specialist"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {messages.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center">
                    ...
                    <p className="text-gray-500 text-center max-w-md">
                      Start the conversation with your doctor. Share your symptoms, ask questions, or discuss your treatment plan.
                    </p>
                    <button
                      onClick={() => setNewMessage("Hello Doctor, I have a question about my appointment.")}
                          ...
                          <div className={`rounded-2xl px-4 py-2 ${
                            ...
                            <div className={`flex justify-between items-center mt-1 ${
                              ...
                              {message.senderType === "patient" && message.read && (
                                <span className="text-xs ml-2">✓ Read</span>
                              )}
                            </div>
                          </div>
                          {message.senderType === "doctor" && (
                            <div className="flex items-center mt-1 ml-1">
                              {/* Doctor Profile Picture in Message - UPDATED */}
                              <div className="relative w-6 h-6 rounded-full overflow-hidden bg-blue-100 flex-shrink-0">
                                {doctor?.profilePicture && !doctorImageError ? (
                                  <img
                                    src={`http://localhost:5000/${doctor.profilePicture}`}
                                    alt={`${doctor.firstName} ${doctor.lastName}`}
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button
                onClick={() => setNewMessage("I'm experiencing pain.")}
              ...
              <button
                onClick={() => setNewMessage("Can you explain my diagnosis?")}
                className="px-3 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition duration-200 text-sm"
              >
                Explain diagnosis
              </button>
              <button
                onClick={() => setNewMessage("What medications should I take?")}
                className="px-3 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition duration-200 text-sm"
              >
                Medication advice
              </button>
              <button
                onClick={() => setNewMessage("When is my next appointment?")}
                className="px-3 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition duration-200 text-sm"
              >
                Next appointment
              </button>
            ...
            {appointment?.patientDetails?.medicalConcern && (
              <div className="mt-6 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h4 className="text-lg font-semibold text-gray-900 mb-3">Medical Context</h4>
                <div className="space-y-3">
                  <div>
                ...
                <p className="text-xs text-gray-500 mt-3">
                  This information helps your doctor provide better advice. Keep your doctor updated about any changes.
                </p>
              </div>
            )}
          </div>
